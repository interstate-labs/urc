...
contract UnitTestHelper is Test {
    ...
    function _hashToLeaves(IRegistry.SignedRegistration[] memory _registrations) internal pure returns (bytes32[] memory) {
    ...
    function getRegistrationData(bytes32 registrationRoot) public view returns (OperatorData memory) {
        IRegistry.OperatorData memory data = registry.getOperatorData(registrationRoot);
        
        return OperatorData({
            owner: data.owner,
            collateralGwei: uint56(data.collateralWei / 1 gwei),
            numKeys: uint8(data.numKeys),
            registeredAt: uint32(data.registeredAt),
            unregisteredAt: uint32(data.unregisteredAt),
            slashedAt: uint32(data.slashedAt)
        });
    }
    ...
    // Helper functions to access registry configuration
    function getMinCollateral() public view returns (uint256) {
        return registry.getConfig().minCollateralWei;
    }
    
    function getFraudProofWindow() public view returns (uint32) {
        return registry.getConfig().fraudProofWindow;
    }
    
    function getUnregistrationDelay() public view returns (uint32) {
        return registry.getConfig().unregistrationDelay;
    }
    
    function getSlashWindow() public view returns (uint32) {
        return registry.getConfig().slashWindow;
    }
    
    function getOptInDelay() public view returns (uint32) {
        return registry.getConfig().optInDelay;
    }
}
...
contract ReentrantSlashableRegistrationContract is ReentrantContract {
    ...
    receive() external payable {
        ...
        try registry.slashRegistration(
